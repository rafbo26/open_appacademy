...
def all_vowel_pairs(words)
    ...
    v_pairs = pairs.select do |pair|
        vowels.all? do |vowel|
            pair.join("").include?(vowel)
        end
    end
    v_pairs.map! { |pair| pair.join(" ") }
...
def get_pairs(words)
    ...
    words.each.with_index do |word_1, i_1|
        words.each.with_index do |word_2, i_2|
            pairs << [word_1, word_2] if i_2 > i_1
        end
    end
    pairs
end
...
class String
    ...
    def caesar_cipher(num)
        alpha = *("a".."z")
        ciph = self.split("").map.with_index { |char, i| alpha[(alpha.index(char) + num) % 26 ] }
        ciph.join("")
    end
end
